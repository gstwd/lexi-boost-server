...
class WordController(BaseController):
    ...
    def _register_routes(self):
        ...
        self.blueprint.add_url_rule('/word-records/check-duplication', 'check_duplication', self.check_duplication,
                                    methods=['POST'])
        self.blueprint.add_url_rule('/word-entries/search', 'search_word_entries', self.search_word_entries,
                                    methods=['GET'])
    ...
    def search_word_entries(self):
        """搜索单词"""
        query = request.args.get('query', '', type=str)
        page = request.args.get('page', 1, type=int)
        limit = request.args.get('limit', 10, type=int)
        words = self._word_service.search_word_entries(query, page, limit)
        return self.success_response(words)
